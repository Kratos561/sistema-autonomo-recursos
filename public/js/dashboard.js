...
async function loadOverview() {
    ...
    `).join('');
        ...
        topContainer.innerHTML = '<div class="loading">Ejecutando primera recolección...</div>';
    }
    ...
    `).join('');
        ...
        alertsContainer.innerHTML = '<div class="loading">Sin alertas aún</div>';
    }
    ...
    `).join('');
        ...
        logsContainer.innerHTML = '<div class="log-item"><span class="log-message">Sistema recién iniciado...</span></div>';
    }
}
...
async function loadResources() {
    ...
    if (!data || data.length === 0) {
        container.innerHTML = '<div class="loading">No hay recursos. Ejecuta el pipeline primero.</div>';
        return;
    }
    ...
    <div class="resource-card" onclick="window.open('${r.url}', '_blank')">
      ...
      <div class="resource-desc">${escapeHtml(r.description || 'Sin descripción')}</div>
      <div class="resource-tags">
        <span class="tag tag-type">${r.type || 'other'}</span>
        <span class="tag tag-domain">${r.domain || 'general'}</span>
        ${!r.credit_card ? '<span class="tag tag-free">sin tarjeta</span>' : ''}
      </div>
      <div class="resource-meta">
        <div class="resource-scores">
          <span>Rareza: ${parseFloat(r.rarity_score || 0).toFixed(0)}</span>
          <span>Valor: ${parseFloat(r.value_score || 0).toFixed(0)}</span>
          <span>Riesgo: ${parseFloat(r.risk_score || 0).toFixed(0)}</span>
        </div>
        <span class="alert-time">${timeAgo(r.discovered_at)}</span>
      </div>
    </div>
  `).join('');
}
...
async function loadSources() {
    ...
    if (!data || data.length === 0) {
        container.innerHTML = '<div class="loading">Sin fuentes configuradas</div>';
        return;
    }

    container.innerHTML = data.map(s => `
    <div class="alert-item">
      <div class="alert-icon">${s.is_active ? '🟢' : '🔴'}</div>
      <div class="alert-content">
        <div class="alert-title">${escapeHtml(s.name)}</div>
        <div class="alert-message">${s.source_type} · Éxito: ${parseFloat(s.success_rate || 100).toFixed(0)}%</div>
      </div>
      <div class="alert-time">${s.last_crawled ? timeAgo(s.last_crawled) : 'pendiente'}</div>
    </div>
  `).join('');
}
